...
function ReservationListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ReservationInterface[]>(
    ...
    () =>
      getReservations({
        relations: ['user_reservation_customer_idTouser', 'restaurant', 'user_reservation_wait_staff_idTouser'],
      }),
  );
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_reservation_customer_idTouser?.id}`}>
                          {record.user_reservation_customer_idTouser?.email}
                        </Link>
                      </Td>
                    )}
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_reservation_wait_staff_idTouser?.id}`}>
                          {record.user_reservation_wait_staff_idTouser?.email}
                        </Link>
                      </Td>
                    )}
